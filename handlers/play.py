...
async def play(_, message: Message):
    global que
    lel = await message.reply("🔄 **Processing Via ✯SavageMusic✯**")
                      ...
                      if administrator == message.from_user.id:  
                          ...
                          try:
                              ...
                              await lel.edit(
                                  ...
                                  "\n\nOr manually add @SAVAGE_USERBOT to your Group and try again</b>",
                              )
                              pass
    try:
        chatdetails = await USER.get_chat(chid)
    ...
    await lel.edit("🔎 **Finding Via ✯SavageMusic✯**")
    sender_id = message.from_user.id
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    await lel.edit("🎵 **Processing Via ✯SavageMusic✯**")
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png",
        reply_markup=keyboard,
        caption="▶️ **Playing** song requested by {} via SavageMusic ".format(
...
async def deezer(client: Client, message_: Message):
    global que
    lel = await message_.reply("🔄 **Processing Via ✯SavageMusic✯**")
    administrators = await get_administrators(message_.chat)
    chid = message_.chat.id
    try:
        user = await USER.get_me()
    except:
        user.first_name =  "SavageMusic"
    usar = user
    wew = usar.id
    try:
        #chatdetails = await USER.get_chat(chid)
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"✯SavageMusic✯= #️⃣ Queued at position {position}")
    else:
        await res.edit_text("✯SavageMusic✯=▶️ Playing.....")
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"✯SavageMusic✯=#️⃣ Queued at position {position}",
